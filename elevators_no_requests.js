{
    // Algorithm: When an elevator is idle, it will go to the best floor where
    //            buttons are still light up. The best floor is the closest one
    //            that does not have another elevator already servicing it.
    //
    ...
    //
    // Average wait time: 12.2s
    // Maximum wait time: 40.1s
    ...
    init: function(elevators, floors) {
        ...
        var flRequestHasElevator = function(floorNum, direction) {
            var found = false;
            elevators.forEach(function(elevator) {
                if (elevator.my_willHaveDirAtFloor(floorNum, direction)) {
                    found = true;
                }
            });
            return found;
        ...
        elevators.forEach(function(elevator) {
            ...
            elevator.my_willHaveDirAtFloor = function(floorNum, direction) {
            ...
            elevator.my_removeDestination = function(floorNum) {
                var queueIndex = elevator.destinationQueue.indexOf(floorNum);
                if (queueIndex < 0)
                    return;
        ...
        var bestRequest = function (elevator) {
            ...
            floors.forEach(function(floor) {
                ...
                if (floor.buttonStates.up) {
                    need = need || !flRequestHasElevator(floorNum, "up");
                }
                if (floor.buttonStates.down) {
                    need = need || !flRequestHasElevator(floorNum, "down");
                }
                if (need) {
                    var reqDist = Math.abs(curFloorNum - floorNum);
                    if (reqDist < smallestDist) {
                        bestFloorNum = floorNum;
                        smallestDist = reqDist;
                    }
                }
            });
            return bestFloorNum;
        ...
        // When a floor button is pressed, use any elevator that is idle.
        floors.forEach(function (floor) {
    ...
    update: function(dt, elevators, floors) {
        ...
        floors.forEach(function(floor) {
            ...
            if (!floor.buttonStates.up) {
                elevators.forEach(function(elevator) {
                    if (!elevator.my_occupantWantToGo(floorNum)) {
                        if (elevator.my_willHaveDirAtFloor(floorNum, "up")) {
                            elevator.my_removeDestination(floorNum);
                        }
                    }
                });
            }
            if (!floor.buttonStates.down) {
                elevators.forEach(function(elevator) {
                    if (!elevator.my_occupantWantToGo(floorNum)) {
                        if (elevator.my_willHaveDirAtFloor(floorNum, "down")) {
                            elevator.my_removeDestination(floorNum);
                        }
                    }
                });
